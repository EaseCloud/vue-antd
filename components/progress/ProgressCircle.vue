    ...
    <div
      ...
      <svg viewBox='0 0 100 100'>
        ...
        <path :d="pathString" stroke-linecap='round'
              :stroke="strokeColor" :stroke-width="strokeWidth" fill-opacity='0'
              :style="pathStyle"/>
      </svg>

      <span v-if="status === 'exception'" :class="prefixCls + '-circle-text'"><v-icon
        type="exclamation"></v-icon></span>
      <span v-if="status === 'success'" :class="prefixCls + '-circle-text'"><v-icon
        type="check"></v-icon></span>
      <span v-if="status !== 'exception' && status !== 'success'"
            :class="prefixCls + '-circle-text'">{{ percent }}%</span>
    </div>
  </div>
</template>
