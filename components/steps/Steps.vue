  ...
  methods: {
    _mapPropsToChildComponent(isInit) {
      ...
      this.$children.forEach((child, index) => {
        ...
        // child.tailWidth = (self.itemsWidth.length === 0 || index === len)
        // ? 'auto' : self.itemsWidth[index] + self.tailWidth + 'px'
...
//      const dom = this.$el;
      ...
      let i = 0;
      for (; i <= len - 1; i++) {
        const item = this.$el.children[i].children;
        this.itemsWidth[i] = Math.ceil(item[0].offsetWidth + item[1].children[0].offsetWidth);
      }
